...
class OpeningUI(QDialog):
    ...
    def init_ui(self):
        ...
        print("dev")
        self.ui.lineEdit.setText(r"D:\A_programs\V3-Edit\mod\2890076329")
        self.ui.lineEdit_2.setText(r"D:\Steam\steamapps\common\Victoria 3\game")
    ...
    def loadv3file(self, QlineEdit1, QlineEdit2):
        ...
        try:
            ...
            BM.setBM(GamePath, ModPath, "rpSAME")
            #print(BM.mods.keys())
            goods, raw_goods = BM.get_part("goods")
            pm, raw_pm = BM.get_part("pm")
            pmg, raw_pmg = BM.get_part("pmgs")
            bg, raw_bg = BM.get_part("buildings")
            ...
            # #忽略mod与game的区别，合并
            # goods = goods + raw_goods
            # pm = pm + raw_pm
            # pmg = pmg + raw_pmg
            # bg = bg + raw_bg

            goods_header = Goods.structure_list()
            main_ui.ui.tableWidget_goods.setHorizontalHeaderLabels(goods_header)
            ...
            for goods_name in goods:
                single_goods, rc = BM.get_part_detail("goods", goods_name)
                single_goods:Goods

                for i in range(len(goods_header)):
                    i_data = single_goods[goods_header[i]]
                    if i_data is None:
                        i_data = ""

                    main_ui.ui.tableWidget_goods.setItem(k, i, QTableWidgetItem(str(i_data)))
            ...
            main_table3.ui.comboBox_3.addItems(goods)
            main_table3.ui.comboBox_2.addItems(goods)
            ...
            main_ui.ui.tableWidget_pm.setHorizontalHeaderLabels(pm_header)
            k = 0
            ...
            pmg_header = Pmg.structure_list()

            main_ui.ui.tableWidget_pmg.setHorizontalHeaderLabels(pmg_header)
            
            k = 0
            for pmg_name in pmg:
                single_pmg, rc = BM.get_part_detail("pmgs", pmg_name)
                single_pmg:Pmg

                for i in range(len(pmg_header)):
                    i_data = single_pmg[pmg_header[i]]
                    text = ''
                    if i_data is None:
                        text = ''
                    #如果是字典，每行放一个key = value字符串
            ...
            for bg_name in bg:
                ...
                for i in range(len(bg_header)):
                    i_data = single_bg[bg_header[i]]
                    text = ''
                    if i_data is None:
                        text = ''
                    #如果是blocklike，转为字符串
                    ...
                    main_ui.ui.tableWidget_bg.setItem(k, i, QTableWidgetItem(text))
            ...
            for i in range(0, row_count - 1):
                if main_ui.ui.tableWidget_pmg.item(i, 0).text() != '':
                    ...
                    if main_ui.ui.tableWidget_pmg.item(i, 3).text() != '':
                        temp = temp + '\tis_hidden_when_unavailable = {}\n'.format(main_ui.ui.tableWidget_pmg.item(i, 3).text())
            ...
            for i in range(0, row_count - 1):
                if main_ui.ui.tableWidget_bg.item(i, 0).text() != '':
                    ...
                    if main_ui.ui.tableWidget_bg.item(i, 5).text() != '':
                        temp = temp + '\tshould_auto_expand = \n{}\n'.format(main_ui.ui.tableWidget_bg.item(i, 5).text())
                    ...
                    if main_ui.ui.tableWidget_bg.item(i, 20).text() != '':
                        temp = temp + '\trequired_construction = {}\n'.format(main_ui.ui.tableWidget_bg.item(i, 20).text())
                    ...
                    if main_ui.ui.tableWidget_bg.item(i, 22).text() != '':
                        temp = temp + '\teconomic_contribution = {}\n'.format(main_ui.ui.tableWidget_bg.item(i, 22).text())
                    ...
                    if main_ui.ui.tableWidget_bg.item(i, 27).text() != '':
                        temp = temp + '\tai_subsidies_weight = {}\n'.format(main_ui.ui.tableWidget_bg.item(i, 27).text())
...
class MainTable_PM(QDialog):
    ...
    def copy(self):
        ...
        text = ''
        for i in range(0, row_out - 1):
            text = text + self.ui.tableWidget_goods.item(i, 0).text() + ' = ' + self.ui.tableWidget_goods.item(i, 1).text() + '\n'
